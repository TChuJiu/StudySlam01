...
{
    return T_c_w_.inverse().translation(); //找相机坐标系下的光心，相机坐标系下（0,0,0），对T_c_w_  求逆矩阵，然后取平行量
}

bool Frame::isInFrame ( const Vector3d& pt_world ) //判断给世界坐标点，是不是在当前帧内
{
    ...
    return pixel(0,0)>0 && pixel(1,0)>0 
        && pixel(0,0)<color_.cols       // 列数
        && pixel(1,0)<color_.rows;      // 行数
}
